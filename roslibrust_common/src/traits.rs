...
use crate::{DynamicMessage, MessageDescriptor, Result, ServiceDescriptor, ServiceError};
...
pub trait RosServiceType: 'static + Send + Sync + Sized {
    /// Runtime metadata and format-independent operations for this service type.
    const DESCRIPTION: ServiceDescriptor = ServiceDescriptor::new::<Self>();
...
pub trait DynamicService {
    /// The generated descriptor this client was created with.
...
/// Describes a backend that can call services using runtime-selected service types.
pub trait DynamicServiceProvider {
    type DynamicServiceClient: DynamicService + Send + Sync + 'static;

    /// Make a one-shot call to a runtime-selected service.
    fn dynamic_call_service(
        &self,
